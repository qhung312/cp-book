...
template <typename P> pair<int, P> line_intersect(P a, P b, P c, P d) {
...
template <typename T> T polygon_area2(vector<Point<T>> &v) {
    T ans = 0;
    int n = v.size();
    for (int i = 0; i < n; i++) {
        ans += v[i].x * v[(i + 1) % n].y;
...
template <typename P> bool on_seg(P a, P b, P p) {
    return collinear(a, b, p) && dot(p - a, b - p) >= 0;
}

// Check if p is inside a polygon
// Has optional parameter for strictly inside
template <typename P> bool inside_polygon(vector<P> &v, P p, bool s = true) {
    int cnt = 0;
    int n = v.size();
...
template <typename P> vector<P> convex_hull(vector<P> v) {
    if (v.size() == 1)
        return v;
    ...
    for (auto p : v) {
        while (ans.size() >= 2 &&
               !ccw(ans[ans.size() - 2], ans[ans.size() - 1], p)) {
            ans.pop_back();
        }
        ans.push_back(p);
    }
...
template <typename P> double seg_dist(P a, P b, P p) {
...
template <typename P> vector<P> seg_intersect(P a, P b, P c, P d) {
    auto bad = cross(b - a, d - c);
    if (bad == 0) {
        if (collinear(a, b, c)) {
            vector<P> ans;
...
// (https://cp-algorithms.com/geometry/minkowski.html) Uses mks_reorder as
// helper function Works in O(n+m)
template <typename P> void mks_reorder(vector<P> &a) {
    int p = 0;
...
template <typename P> vector<P> minkowski(vector<P> a, vector<P> b) {
    mks_reorder(a);
    mks_reorder(b);
    a.push_back(a[0]);
    a.push_back(a[1]);
