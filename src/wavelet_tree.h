...
- count_in_range(x, y, i, j): number of elements in range [i, j] with value in range [x, y]
- k_sorted(k, i, j): value of k-th element (0-indexed) after sorting range [i, j]
...
struct wavelet_tree {
	T l, r;
	wavelet_tree<T>* lt; wavelet_tree<T>* rt;
	vector<T> a;
	vector<int> mleft, mright;
	wavelet_tree(const vector<T>& _a, T _l, T _r) : a(_a), l(_l), r(_r), lt(nullptr), rt(nullptr) {
		T s = (l + r) / 2;
		int n = a.size();
		mleft.resize(n); mright.resize(n);
		int cntl = 0, cntr = 0;
		vector<T> lv, rv;
		for (int i = 0; i < n; i++) {
			if (a[i] <= s) {
		...
		if (l != r) {
			...
			if (!lv.empty()) lt = new wavelet_tree<T>(lv, l, s);
			if (!rv.empty()) rt = new wavelet_tree<T>(rv, s + 1, r);
		}
	}

	void clear() {
		if (lt) { lt->clear(); delete lt; lt = nullptr; }
		if (rt) { rt->clear(); delete rt; rt = nullptr; }
	}
	~wavelet_tree() { clear(); }
	...
	int count_leq(T x, int i) {
		...
		T s = (l + r) / 2;
		int ans = 0;
		if (lt) ans += lt->count_leq(x, mleft[i]);
		if (rt) ans += rt->count_leq(x, mright[i]);
		return ans;
	}

	int count_leq(T x, int i, int j) {
		int ans = count_leq(x, j);
		if (i > 0) ans -= count_leq(x, i - 1);
		return ans;
	}
